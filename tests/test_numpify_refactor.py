...
def test_identifier_mangling_and_collision() -> None:
    ...
    f = mod.numpify(a + b + c1 + c2, vars=(a, b, c1, c2), cache=False)
...
def test_dynamic_parameter_context_and_unfreeze() -> None:
    ...
    x, a = sp.symbols("x a")
    f = mod.numpify(a * x, vars=(x, a), cache=False)
    ctx = _Ctx({a: 2.0})
...
def test_unfreeze_without_keys_unfreezes_all_nonfree_vars() -> None:
    ...
    bound = f.set_parameter_context(ctx).freeze(
        {a: mod.DYNAMIC_PARAMETER, b: mod.DYNAMIC_PARAMETER}
    )
...
def test_dynamic_missing_context_errors() -> None:
    ...
    x, a = sp.symbols("x a")
    f = mod.numpify(a * x, vars=(x, a), cache=False).freeze({a: mod.DYNAMIC_PARAMETER})

...
from .parameter_keys import ParameterKeyOrKeys, expand_parameter_keys_to_symbols, parameter_name
...
from .figure_legend import LegendPanelManager, LegendModel
from .figure_parameters import ParameterManager, ParameterPanel
    ...
    **_deprecated_kwargs : Any, optional
    ...
    def __init__(
        ...
        **_deprecated_kwargs: Any,
        ...
        self._parameter_panels: list[ParameterPanel] = []
        self._legend_panels: list[LegendPanelManager] = []
        ...
        self._parameter_panel = self.create_parameter_panel(
            plot_widget=self.pane.widget,
            root_widget=self._layout.params_box,
            register=True,
        )
    ...
    def add_view(
        ...
        *,
        ...
        if view.is_active:
            ...
            self._sync_default_presentations_for_plot_widget(view.pane.widget)
        else:
    ...
    def set_active_view(self, id: str) -> None:
        ...
        if current_view.id == view_id:
            ...
            self._sync_default_presentations_for_plot_widget(self.views[view_id].pane.widget)
        ...
        self._sync_default_presentations_for_plot_widget(nxt.pane.widget)
    ...
    def remove_view(self, id: str) -> None:
        ...
        for plot in self.plots.values():
            ...
            self._notify_plot_updated(plot)
        ...
        self._layout.set_view_order(tuple(self._view_manager.views.keys()))
        self._layout.set_active_view(self.views.current_id)
        self._info.set_active_view(self.views.current_id)
        if self._view_manager.views:
            self._sync_default_presentations_for_plot_widget(self.pane.widget)
        if self._sync_sidebar_visibility():
            self._request_active_view_reflow("sidebar_visibility")
        else:
    ...
    def _sync_sidebar_visibility(self) -> bool:
        ...
        params_visible = (
            self._parameter_panel.panel_visible
            if hasattr(self, "_parameter_panel") and self._parameter_panel is not None
            else self._parameter_manager.has_params
        )
        info_visible = (
            self._info.default_group_has_info
            if hasattr(self._info, "default_group_has_info")
            else self._info.has_info
        )
        ...
        self._emit_layout_event("sidebar_visibility_sync", source="Figure", phase="completed", changed=changed, params_visible=params_visible, info_visible=info_visible, legend_visible=self._legend.panel_visible)
        return changed
    ...
    def _active_plot_widget(self) -> Any | None:
        """Return the currently active Plotly mount widget when available."""
        if not self._view_manager.views:
            return None
        return self.views.current.pane.widget

    def _resolve_plot_widget_view_ids(self, plot_widget: Any) -> tuple[str, ...]:
        """Resolve a plot widget/container-like object back to concrete view ids."""
        if plot_widget is None:
            return ((self.views.current_id,) if self._view_manager.views else ())
        if isinstance(plot_widget, str):
            view_id = str(plot_widget)
            return (view_id,) if view_id in self._view_manager.views else ()

        matches: list[str] = []
        for view_id, view in self._view_manager.views.items():
            if plot_widget is view:
                matches.append(view_id)
                continue
            if plot_widget is view.pane:
                matches.append(view_id)
                continue
            if plot_widget is getattr(view.pane, "widget", None):
                matches.append(view_id)
                continue
            if plot_widget is view.figure_widget:
                matches.append(view_id)
                continue
        return tuple(matches)

    def plots_for_plot_widget(self, plot_widget: Any) -> tuple[Any, ...]:
        """Auto-generated reference note for ``plots_for_plot_widget``.

        Full API
        --------
        ``plots_for_plot_widget(...)``
        ...
        """
        """Return the global plots shown on the given Plotly widget/container."""
        target_view_ids = set(self._resolve_plot_widget_view_ids(plot_widget))
        if not target_view_ids:
            return ()
        matched: list[Any] = []
        for plot in self.plots.values():
            plot_view_ids = {str(view_id) for view_id in getattr(plot, "views", ())}
            if target_view_ids & plot_view_ids:
                matched.append(plot)
        return tuple(matched)

    def parameters_for_plot_widget(self, plot_widget: Any) -> tuple[str, ...]:
        """Auto-generated reference note for ``parameters_for_plot_widget``.

        Full API
        --------
        ``parameters_for_plot_widget(...)``
        ...
        - Start with ``docs/guides/api-discovery.md`` for package navigation.
        """
        """Return parameter names used by plots shown on the given widget."""
        ordered: list[str] = []
        seen: set[str] = set()
        for plot in self.plots_for_plot_widget(plot_widget):
            for raw_parameter in getattr(plot, "parameters", ()):
                try:
                    name = parameter_name(raw_parameter, role="parameter")
                except Exception:
                    name = getattr(raw_parameter, "name", str(raw_parameter))
                if name not in seen:
                    ordered.append(name)
                    seen.add(name)
        return tuple(ordered)

    def _plot_filter_for_widget(self, plot_widget: Any) -> Callable[[Any], bool]:
        """Build an explicit plot-membership filter tied to one widget target."""
        return lambda plot, _widget=plot_widget: any(
            str(view_id) in set(self._resolve_plot_widget_view_ids(_widget))
            for view_id in getattr(plot, "views", ())
        )

    def _parameter_filter_for_widget(self, plot_widget: Any) -> Callable[[], tuple[str, ...]]:
        """Build an explicit parameter-membership filter tied to one widget target."""
        return lambda _widget=plot_widget: self.parameters_for_plot_widget(_widget)

    def create_parameter_panel(
        self,
        *,
        plot_widget: Any | None = None,
        parameter_filter: Callable[[], Sequence[str]] | Sequence[str] | None = None,
        root_widget: widgets.Box | None = None,
        register: bool = True,
    ) -> ParameterPanel:
        """Auto-generated reference note for ``create_parameter_panel``.
        ...
        - Start with ``docs/guides/api-discovery.md`` for package navigation.
        """
        """Create a filtered parameter presentation over the global parameter store."""
        effective_filter = parameter_filter
        if effective_filter is None and plot_widget is not None:
            effective_filter = self._parameter_filter_for_widget(plot_widget)
        panel = self._parameter_manager.create_panel(
            parameter_filter=effective_filter,
            root_widget=root_widget,
            modal_host=self._layout.root_widget,
        )
        if register and panel not in self._parameter_panels:
            self._parameter_panels.append(panel)
        return panel
    ...
    def create_legend_panel(
        ...
        *,
        ...
        """Auto-generated reference note for ``create_legend_panel``.
        ...
        - Start with ``docs/guides/api-discovery.md`` for package navigation.
        """
        """Create a filtered legend presentation over the global plot store."""
        effective_filter = plot_filter
        if effective_filter is None and plot_widget is not None:
            effective_filter = self._plot_filter_for_widget(plot_widget)
        panel = LegendPanelManager(
            layout_box or widgets.VBox(),
            modal_host=self._layout.root_widget,
            enable_plot_editor=enable_plot_editor,
            model=LegendModel(plot_filter=effective_filter),
        )
        for plot in self.plots.values():
            panel.on_plot_added(plot)
        if register and panel not in self._legend_panels:
            self._legend_panels.append(panel)
        return panel

    def _sync_default_presentations_for_plot_widget(self, plot_widget: Any | None) -> None:
        """Retarget the default legend/parameter presentations to one visible widget."""
        target_widget = self._active_plot_widget() if plot_widget is None else plot_widget
        self._legend.set_plot_filter(self._plot_filter_for_widget(target_widget))
        if hasattr(self, "_parameter_panel") and self._parameter_panel is not None:
            self._parameter_panel.set_parameter_filter(
                self._parameter_filter_for_widget(target_widget)
            )
        self._refresh_parameter_presentations()

    def _refresh_parameter_presentations(self) -> None:
        """Refresh all registered parameter panels after plot membership changes."""
        for panel in tuple(self._parameter_panels):
            panel.refresh()

    def _iter_legend_panels(self) -> tuple[LegendPanelManager, ...]:
        """Yield the default legend plus any additional filtered legend panels."""
        extras = [panel for panel in self._legend_panels if panel is not self._legend]
        return (self._legend, *extras)

    def _notify_plot_added(self, plot: Any) -> None:
        """Notify every legend presentation that a plot was added."""
        for panel in self._iter_legend_panels():
            panel.on_plot_added(plot)
        self._refresh_parameter_presentations()

    def _notify_plot_updated(self, plot: Any) -> None:
        """Notify every legend presentation that a plot changed."""
        for panel in self._iter_legend_panels():
            panel.on_plot_updated(plot)
        self._refresh_parameter_presentations()

    def _notify_plot_removed(self, plot_id: str) -> None:
        """Notify every legend presentation that a plot was removed."""
        for panel in self._iter_legend_panels():
            panel.on_plot_removed(plot_id)
        self._refresh_parameter_presentations()

"""Public pane objects and the figure-level pane facade.

A :class:`FigurePane` is the public per-workspace object returned by
``fig.panes[pane_id]``. It owns one stable Plotly widget runtime together with
per-pane axis defaults, remembered viewport state, and selector metadata.

``FigurePanes`` is the small mapping-like facade exposed on :class:`Figure`.
It provides ``current`` / ``current_id`` helpers plus creation and removal
delegation without exposing the figure's internal pane registry details.
"""
...
class FigurePane:
    ...
    Full API
    --------
    ``FigurePane(figure: Figure, id: str, title: str, x_label: str, y_label: str, default_x_range: RangeLike, default_y_range: RangeLike, figure_widget: go.FigureWidget, pane: PlotlyPane, is_active: bool=False, is_stale: bool=False)``
    ...
    default_x_range : RangeLike
        Default x-axis range used when a pane is created or reset. Required.
    
    default_y_range : RangeLike
        Default y-axis range used when a pane is created or reset. Required.
    ...
    -------
    FigurePane
        New ``FigurePane`` instance configured according to the constructor arguments.
    ...
    Architecture note
    -----------------
    ``FigurePane`` lives in ``gu_toolkit._figure_panes``. The figure layer is coordinator-driven: Figure owns orchestration, while pane/layout/info/parameter collaborators own their specific state. Use the class as the stable owner for this slice of state rather than reaching into collaborators directly.
    
    Examples
    --------
    Construction::
    
        from gu_toolkit._figure_panes import FigurePane
        obj = FigurePane(...)
    ...
    - Runtime discovery tip: use ``with fig:`` or ``with fig.panes["id"]:`` and inspect ``help(Figure)`` for the class-based and current-figure surfaces.
    - In a notebook or REPL, run ``help(FigurePane)`` and ``dir(FigurePane)`` to inspect adjacent members.
    """
    ...
    def figure(self) -> Figure:
        """Return the parent figure that owns this pane.
        ...
        Architecture note
        -----------------
        This member belongs to ``FigurePane``. The figure layer is coordinator-driven: Figure owns orchestration, while pane/layout/info/parameter collaborators own their specific state. Use it through the owning object rather than bypassing the surrounding figure/runtime machinery.
    ...
    def figure_widget(self) -> go.FigureWidget:
        """Return the stable Plotly widget owned by this pane.
    ...
    def pane(self) -> PlotlyPane:
        """Return the stable :class:`PlotlyPane` owned by this pane.
    ...
    def plotly_layout(self) -> Any:
        """Return the Plotly layout object for this pane's widget.
    ...
    @property
    def title(self) -> str:
        """Return the selector title shown for this pane.
    ...
    def x_label(self) -> str:
        """Return the stored x-axis label for this pane.
    ...
    def x_label(self, value: str) -> None:
        """Work with x label on ``FigurePane``.
    ...
    def y_label(self) -> str:
        """Return the stored y-axis label for this pane.
    ...
    def y_label(self, value: str) -> None:
        """Work with y label on ``FigurePane``.
    ...
    def x_range(self) -> tuple[float, float]:
        """Return the default x-range for this pane.
    ...
    def x_range(self, value: RangeLike) -> None:
        """Work with x range on ``FigurePane``.
    ...
    def default_x_range(self, value: RangeLike) -> None:
        """Work with default x range on ``FigurePane``.
    ...
    def y_range(self) -> tuple[float, float]:
        """Return the default y-range for this pane.
    ...
    def y_range(self, value: RangeLike) -> None:
        """Work with y range on ``FigurePane``.
    ...
    def default_y_range(self, value: RangeLike) -> None:
        """Work with default y range on ``FigurePane``.
    ...
    def current_x_range(self, value: RangeLike | None) -> None:
        """Work with current x range on ``FigurePane``.
    ...
    def current_y_range(self, value: RangeLike | None) -> None:
        """Work with current y range on ``FigurePane``.
    ...
    def activate(self) -> FigurePane:
        """Activate this pane on its parent figure and return ``self``.
        
        Full API
        --------
        ``obj.activate() -> FigurePane``
        ...
        - In a notebook or REPL, run ``help(FigurePane)`` and ``dir(FigurePane)`` to inspect adjacent members.
        """
        self._figure.select_pane(self.id)
        return self

    def __enter__(self) -> FigurePane:
        previous: str | None = None
        try:
            previous = self._figure.panes.current_id
    ...
    def __exit__(self, exc_type: Any, exc: Any, tb: Any) -> None:
        ...
        try:
            if previous is not None and previous in self._figure.panes and previous != self.id:
                self._figure.select_pane(previous)
        finally:
            self._figure.__exit__(exc_type, exc, tb)


class FigurePanes(Mapping[str, FigurePane]):
    """Mapping-like facade that exposes a figure's registered panes.
    
    Full API
    --------
    ``FigurePanes(fig: Figure)``
    ...
    -------
    FigurePanes
        New ``FigurePanes`` instance configured according to the constructor arguments.
    ...
    Architecture note
    -----------------
    ``FigurePanes`` lives in ``gu_toolkit._figure_panes``. The figure layer is coordinator-driven: Figure owns orchestration, while pane/layout/info/parameter collaborators own their specific state. Use the class as the stable owner for this slice of state rather than reaching into collaborators directly.
    
    Examples
    --------
    Construction::
    
        from gu_toolkit._figure_panes import FigurePanes
        obj = FigurePanes(...)
    ...
    - Example notebook: ``examples/Toolkit_overview.ipynb``.
    - Runtime discovery tip: use ``with fig:`` or ``with fig.panes["id"]:`` and inspect ``help(Figure)`` for the class-based and current-figure surfaces.
    - In a notebook or REPL, run ``help(FigurePanes)`` and ``dir(FigurePanes)`` to inspect adjacent members.
    """
    ...
    def __getitem__(self, key: str) -> FigurePane:
        return self._fig._require_pane(str(key))

    def __iter__(self) -> Iterator[str]:
        return iter(self._fig._ordered_pane_ids)

    def __len__(self) -> int:
        return len(self._fig._panes)

    @property
    def current_id(self) -> str:
        """Return the current active pane id.
        ...
        Architecture note
        -----------------
        This member belongs to ``FigurePanes``. The figure layer is coordinator-driven: Figure owns orchestration, while pane/layout/info/parameter collaborators own their specific state. Use it through the owning object rather than bypassing the surrounding figure/runtime machinery.
        ...
        - In a notebook or REPL, run ``help(FigurePanes)`` and ``dir(FigurePanes)`` to inspect adjacent members.
        """
        return self._fig.current_pane_id

    @current_id.setter
    def current_id(self, value: str) -> None:
        """Work with current id on ``FigurePanes``.
        ...
        self._fig.select_pane(str(value))

    @property
    def current(self) -> FigurePane:
        """Return the current active public pane object.
        
        Full API
        --------
        ``obj.current -> FigurePane``
        ...
        - In a notebook or REPL, run ``help(FigurePanes)`` and ``dir(FigurePanes)`` to inspect adjacent members.
        """
        return self._fig._require_pane(self._fig.current_pane_id)
    ...
    def add(
        ...
        *,
        ...
        """Create a new pane by delegating to :meth:`Figure.add_pane`.
        
        Full API
        --------
        ``obj.add(pane_id: str, *, title: str | None=None, x_range: RangeLike | None=None, y_range: RangeLike | None=None, x_label: str | None=None, y_label: str | None=None, activate: bool=False) -> FigurePane``
        
        Parameters
        ----------
        pane_id : str
            Identifier for the relevant pane inside a figure. Required.
    ...
    def remove(self, pane_id: str) -> None:
        """Remove a pane by delegating to :meth:`Figure.remove_pane`.
        
        Full API
        --------
        ``obj.remove(pane_id: str) -> None``
        ...
        - In a notebook or REPL, run ``help(FigurePanes)`` and ``dir(FigurePanes)`` to inspect adjacent members.
        """
        self._fig.remove_pane(pane_id)

    def select(self, pane_id: str) -> None:
        """Select a pane by setting :attr:`current_id`.
        
        Full API
        --------
        ``obj.select(pane_id: str) -> None``
        ...
        - In a notebook or REPL, run ``help(FigurePanes)`` and ``dir(FigurePanes)`` to inspect adjacent members.
        """
        self.current_id = pane_id


__all__ = ["FigurePanes", "FigurePane"]

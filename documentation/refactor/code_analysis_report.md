...
`numpify.py` provides a thoughtful SymPy-to-NumPy compiler:
- LRU caching (`numpify_cached`) avoids redundant compilation on slider drags.
- `DYNAMIC_PARAMETER` sentinel enables deferred binding — parameters can be frozen at compile time or supplied at call time.
